...
//
// `suffix` es para el dato que nunca debe truncarse (p.ej. el % al final de
// "Nombre largo · +12%"): sin él, un nombre largo se comía el propio número
// que la tarjeta pretende destacar. Solo `value` se trunca con ellipsis.
export function TickerTile({ label, value, suffix, valueColor, trend, trendColor }: { label: string; value: string; suffix?: string; valueColor?: string; trend?: number[]; trendColor?: string }) {
    ...
    <div className="min-w-0 rounded-2xl border border-border bg-card p-3.5 transition-colors card-glow">
      ...
      <div className="mt-1.5 flex items-end justify-between gap-2">
        <span className="flex min-w-0 items-baseline gap-1">
          ...
          {suffix && <span className="shrink-0 text-lg font-bold tabular-nums" style={{ color: valueColor }}>{suffix}</span>}
        </span>
        {data && data.length > 1 && (
