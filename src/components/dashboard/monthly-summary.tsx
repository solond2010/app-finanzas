...
function AnimatedValue({ value, prefix = "", suffix = "", color }: { value: number; prefix?: string; suffix?: string; color?: string }) {
  const animated = useAnimatedNumber(value)
  return (
    <span className="text-[28px] font-bold tracking-tight tabular-nums leading-none" style={{ color }}>
...
function StatCard({
  ...
  return (
    <div
      className="stagger-fade relative overflow-hidden rounded-2xl bg-card/70 backdrop-blur-xl p-5 shadow-sm ring-1 ring-border/20 transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-md hover:shadow-primary/5"
    ...
    >
      ...
      {rawValue !== undefined ? (
        <AnimatedValue value={rawValue} prefix={prefix} suffix={suffix} color={color} />
      ) : (
        <p className="text-[28px] font-bold tracking-tight tabular-nums leading-none" style={{ color }}>{value}</p>
      )}
...
export function MonthlySummary({ selectedMonth }: { selectedMonth?: string }) {
  ...
  return (
    <>
      ...
      <div className="stagger-fade col-span-full md:col-span-7" style={{ animationDelay: "400ms" }}>
        <Card>
      ...
      <div className="stagger-fade col-span-full md:col-span-5" style={{ animationDelay: "500ms" }}>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-semibold flex items-center gap-2">
